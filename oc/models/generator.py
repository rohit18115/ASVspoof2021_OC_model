...
class Generator(Model):
    ...
    def __init__(self, ninputs, fmaps,
                 kwidth, poolings, # kwidth is the filter length
                 dec_fmaps=None,
                 dec_kwidth=None,
                 dec_poolings=None,
                 z_dim=None,
        ...
        for pi, (fmap, pool, kw) in enumerate(zip(fmaps, poolings, kwidth),
            ...
            if skip and pi < len(fmaps):
                ...
                gskip = GSkip(skip_type, fmap,
                              ...
                              kwidth=skip_kwidth, # as mentioned in tran.py the skip_width is 11 that is used as the length of the conv kernel used as skip connection
                              bias=bias)
                l_i = pi - 1
