...
def eval_composite(clean_utt, Genh_utt, uttname, noisy_utt=None):
    clean_utt = clean_utt.reshape(-1)
    Genh_utt = Genh_utt.reshape(-1)
    # print("---------------clean_utt: {}, Genh_utt: {}".format(clean_utt.shape, Genh_utt.shape))
...
def PESQ(ref_wav, deg_wav):
    ...
    try:
        print("--------------------------")
        print('ref_tfl: {} deg_tfl: {}'.format(ref_tfl,deg_tfl))
        print('pesqmain'.format(curr_dir))
...
def CompositeEval(ref_wav, deg_wav, uttname, log_all=False):
    # returns [sig, bak, ovl]
    alpha = 0.95
    ...
    # Compute the PESQ
    # print("----------------------ref_wav: {}, deg_wav: {}, uttname: {}".format(ref_wav.shape, deg_wav.shape, uttname))
...
#---------------------------test of the added PESQ----------------
# ref_wav = np.ones((16384))
# deg_wav = np.ones((16384))
# CompositeEval(ref_wav, deg_wav)